...
const ConnectedProgramContentCollection: React.FC<CraftProgramContentCollectionProps> = ({ source, ...rest }) => {
  ...
  const resolvedSource = useMemo<ProgramContentCollectionSource>(() => source ?? DEFAULT_SOURCE, [source])
  const { data, loading, error } = useProgramContentCollection(resolvedSource)
  const defaultCategoryIds = resolvedSource.from === 'recentWatched' ? resolvedSource.defaultCategoryIds : undefined
...
export const CraftProgramContentCollection: UserComponent<PropsWithCraft<CraftProgramContentCollectionProps>> =
