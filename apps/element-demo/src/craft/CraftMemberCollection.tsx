...
export type CraftMemberCollectionProps = Omit<
  MemberCollectionProps,
  'members' | 'isFetching' | 'fetchError'
> & {
...
const ConnectedMemberCollection: React.FC<CraftMemberCollectionProps> = ({ source, ...rest }) => {
  ...
  const resolvedSource = useMemo<MemberCollectionSource>(
...
export const CraftMemberCollection: UserComponent<PropsWithCraft<CraftMemberCollectionProps>> = Craftize(
  ConnectedMemberCollection,
)
